...
## old ### set the the training data to be those rows not matching the index list
## old ### training <- rescaled_df[-test_idx,]

# using only 2012 for the training data and selecting only dropped, grade and gpa
training <- rescaled_df %>% filter(year==0) %>% select(dropped, grade,gpa)
## old ### set the the test data to be those rows matching the index list
## old ### testing <- rescaled_df[test_idx,] 

# using years except 2012 for the test data and selecting only dropped, grade and gpa
testing <- rescaled_df %>% filter(year!=0) %>% select(dropped, grade,gpa)
```
...
set.seed(1842)
# Define the formula
form <- paste(names(testing[1]), 
              paste(names(testing[-1]), 
                    collapse='+'),sep = '~') %>% 
  as.formula()
# show the formula
 form
...
diag.new.tree <- prune(diag.tree, cp = 0.065)
...
Pruned tree
```{r treePredictPruned, warning=FALSE, error=TRUE, message=FALSE}
# use the decision tree created above to predict values in the test data 
# and then store the results
